...
void	display_nice_map(char **map, int nrows, int ncols)
{
	int i;
...
void	update_quick_map(char **map, char ch, sqre a)
{
	int i;
	int j;
...
sqre	size_map(mem b, char *defs, int nrows, int ncols)
{
	int		i;
	int		j;
	sqre	a;
		...
		while (j < ncols)
		{
			if (b.arr[i][j] == defs[0])
				b.sizearr[i][j] = (i == 0 || j == 0) ? 1 : 1 + 
					find_smallest(b.sizearr[i][j - 1], b.sizearr[i - 1][j], b.sizearr[i - 1][j - 1]);
	...
	display_nice_map(b.arr, nrows, ncols);
	return (a);
}
